...
public sealed class Graph : IGraph
{
    ...
    public IGraph Reduce(ICollection<INode> includedNodes, ICollection<INode> excludedNodes)
    {
        ...
        var waypointsInExcluded = includedNodes.Intersect(excludedNodes).ToList();
        ...
        var focusedNodes = new HashSet<INode>(includedNodes);
        var resolutionStack = new Stack<INode>();

        // Process each waypoint to find dependencies
        foreach (var waypoint in includedNodes)
        {
    ...
    public ICollection<IRoute> Resolve(ICollection<INode> includedNodes, int maxRoutes = 50) =>
        Resolve(includedNodes, [], maxRoutes);

    /// <summary>
    /// Resolve valid routes through the graph respecting mutations and requirements.
    /// </summary>
    /// <param name="includedNodes">Nodes that must be visited in this order.</param>
